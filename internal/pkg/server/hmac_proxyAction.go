	...
	case hmacProxyUserNowPlayingRegex.MatchString(request.ApiIdentifier):
		{
			...
			usernowplaying, err := client.GetUserCurrentTrack(request.ApiParameters.Username, proxyConfig.LastFMAPIKey, request.ApiParameters.FetchMusicBrainz)
			if handleError(err, c) {
				return
			}
	...
	case hmacProxyUserNowPlayingEmbed.MatchString(request.ApiIdentifier):
		{
			...
			ct, err := client.GetUserCurrentTrack(request.ApiParameters.Username, proxyConfig.LastFMAPIKey, false)
			if ct == nil || err != nil {
